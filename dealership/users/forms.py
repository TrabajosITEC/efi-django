...
class UserRegisterForm(UserCreationForm):
    ...
    class Meta:
        ...
        widgets = {
            'username': forms.TextInput(attrs={'class': 'form-control m-3','placeholder': 'Nombre de usuario'}),
            'email': forms.EmailInput(attrs={'class': 'form-control m-3', 'required':'required','placeholder': 'Correo Electronico'}),
    ...
    def save(self, commit=True): # Este es el commit del if
            user = super().save(commit=False) # Este lo uso para que no guarde y poder hacerle modificaciones con cosas que haya definido mas arriba
